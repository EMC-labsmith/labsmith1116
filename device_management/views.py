...
def want_dev(id, currentuser):
    ...
    machine = device.name
    subprocess.call(["/home/joe/labsmith_all/labsmith0901Modify/wantmail.sh",curUser,machine,mailto])
...
def release_dev(id, currentuser):
    ...
    if device.owner != currentuser:
        ...
        usagelog = UsageLog.objects.get(machineName=device.name, user=currentuser, isUse='f')
        usagelog.releaseTimestamp = timezone.now()
        usagelog.isUse = 't'
        usagelog.save()
        ...
        if device.wanted != None:
            ...
            # msg.content_subtype = "html"
            # msg.send(fail_silently=False)
            subprocess.call(["/home/joe/labsmith_all/labsmith0901Modify/releasemail.sh",machine,mailto])
            device.wanted = None
        device.user = ""  
        device.save();
